#!/usr/bin/env node
// The headless worker: claim a job, render it in a real browser, report back. It renders through the page's own export door and encodes through the server's
// own socket, so neither is reimplemented here. The renderer class is read from the browser this worker will actually render in, never configured.
...
// A heartbeat unanswered by the time the next one is due has already failed, while a claim or a finish report is worth waiting out. undici's default header timeout is around 300s, so an outage that drops packets without an RST would leave the budget below counting something other than seconds.
...
// `channel: 'chromium'` and not the bundled headless shell, which has no GPU and falls back to SwiftShader - the software rasteriser the class guard below refuses.
...
try {
  ...
  // The recorder rather than the root, which is the main menu now. This load exists only to read the renderer class off a page with a WebGL context, and the menu has none.
  ...
  /**
   * The effect packages this worker's server holds, read once per job - off `/effects`, which is the route the registry itself assembles from.
   *
   * Per job and not once at start: a worker takes up to sixteen jobs, `PUT /effects/:id` happens to a running server, and a package retuned mid-drain would
   * have the skew line quote a build that was replaced an hour ago into the log somebody reads to decide whether a file is a render of what they asked for.
   *
   * A read that did not work is never an empty store: `.json()` on a 500 parses `{"error":"..."}` perfectly well and `?? []` read that as nothing installed,
   * so the gate below refused the job naming a package the machine has. Status and shape are both checked, and anything short of a listing throws.
   *
   * Retried, because this runs inside the claim, so a transport failure here is a job going terminal as `failed`. Four attempts about ten seconds of trying,
   * comfortably inside the queue's two-minute silence window, with a timeout per attempt for the reason the heartbeat has one.
   */
  ...
  const readInstalledEffects = async () => {
    ...
    // Its own sentence, and the one thing it must never be is the sentence below about a package this worker has not got: the two send whoever reads the queue to two different machines.
    throw new Error(
  ...
  /**
   * Whether this worker can render a job at all, answered off the job envelope before a page is opened. A second gate over the condition `exportClip` refuses on,
   * and what it buys is the sentence and the cost: a refusal here names the effects and versions the envelope declares, before a minute of GPU produces the identical
   * refusal from the other end. `jobs-check` asserts which refusal a job came back with, so the two are separable by a run. An absent `requires` is nothing required.
   */
  ...
  /**
   * A job names its capture by content hash and the page opens a take by id, so this is where one becomes the other. By hash and never by id: an id is a
   * filename, two machines can hold different footage under the same one, and a lookup by id would render whatever happened to be called that and look like it worked.
   */
  ...
  while (claimed < MAX) {
    ...
    if (claim.status === 409 || claim.status >= 500) {
      // Work exists and none of it is ours, or the queue itself failed. Reported rather than slept on: a worker that quietly polled forever would turn the scheduling failure back into silence.
    ...
    try {
      // Inside the try, so a server that cannot be read is this job coming back `failed` naming the read rather than the worker dying before its first claim.
      ...
      // Said out loud and then rendered anyway: a version is a string a package author writes, nothing in it says which direction is compatible, and refusing here would make every retune a wall in front of every queued job. The silence is what is not acceptable.
      ...
      // Reopened per job rather than once, because two jobs in a queue are two edits and nothing says they are against the same footage.
      ...
      // Attest the footage and renderer before rendering: the capture is named by content hash, and the renderer class is pinned on the claim.
      ...
      // A render runs for minutes by design, so nothing can time a job out on duration - what the queue expires is silence, and this is the noise. The beat used to clear
      // its interval on any rejection and never re-arm, so one `ECONNRESET` left a still-rendering job quiet for life while `JobStore.requeue` handed it to a second worker.
      // Seven failures at the default 15s beat is 105s against the queue's 120s `STALE_MS`, deliberately inside it, and each beat carries a timeout or the arithmetic is a fiction.
      const BEAT_BUDGET = 7;
      let missed = 0;
      ...
      // The project travels in the job rather than by name: a name would resolve to whatever is in the store when the worker gets round to it, which is the opposite of reproducing an edit.
      const result = await page.evaluate(async (j) => {
        // `restoreProject` rather than `loadProject`: the second fetches by name from the store, and a job carries its document precisely so it does not depend on what the store holds.
        globalThis.__kinect.library.restoreProject(j.project);
        // Through `applyDeliverable`, which is the door, rather than `setActiveDeliverable` past it: the bare assignment skips the version gate and the refusal of a stored size belonging to another shape. Older jobs carry explicit width/height/fps/codec, so those override when no deliverable is present.
        if (j.deliverable) globalThis.__kinect.library.applyDeliverable(j.deliverable);
        // Settled before exporting, or the restore's own repaint lands inside the export's first seek: `ExportTransport` throws on any program position reaching
        // the sink more than once, and it showed up as `the render at 0.000000s reached the export 2 times` on some runs and not others. Then a seek, because
        // `restoreProject` leaves the transport where it was rather than where the restored document says - awaiting `settled()` alone narrowed nothing.
      ...
      // The frame count travels with the outcome, and `server/export.js` refuses a stream whose count differs from the one the export declared. Stopped before
      // the report rather than after it: a beat still in the air when `finish` lands reads the job as `done` and is answered 409, which this loop treats as a revoked lease.
      stopBeating();
