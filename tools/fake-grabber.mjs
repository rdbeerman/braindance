...
// Read from where the band that reads it lives rather than copied: a second producer with its own literal would go on stamping last year's generation into every take the suite plants.
import { CAPTURE_FORMAT } from '../web/format.js';

const argv = process.argv.slice(2);

// One table of every argument this fixture knows. `value` says whether an entry takes a following token; `ignored` marks one it reads and deliberately does nothing with, because refusing it would reject a spawn the server legitimately performs.
const ARGUMENTS = {
  '--source': { value: true },
  '--fps': { value: true },
...
// Both readers go through the table, so a name absent from it is a mistake in this file rather than in the argv.
const argument = (name) => {
...
// Reported, never refused: `buildArgs` may append a flag this fixture cannot honour and that spawn must not fail, but a flag silently dropped is how the colour-off half of the live path stayed unreachable by every proof tool here.
...
// Frames written back to back behind the hello, before the cadence starts. Spaced by a timer every frame is its own turn of the event loop, so a recorder that opened its file one turn late would still catch all but the first.
const BURST = Number(flag('--burst', '0'));
const FRAMES = Number(flag('--frames', '0'));
const TAG = flag('--tag', '');
// One line per message, `type length sha256-of-payload`, so a take can be checked against the writer rather than against the reader - polling the library would make it scan the take being written and measure an artifact its own question created.
const EMIT_LOG = flag('--emit-log', '');
// Whether this writer can produce type 3, the native-resolution colour the webcam output reads. The frame is the registered image
// upscaled plus a marker in the outer margin: the colour camera sees 84.1 degrees where the registered frustum sees 70.6, so an
// implementation that cheats by scaling type 2 matches almost the whole picture and still cannot produce the margin.
const HD = given('--hd');
const COLOR = !given('--no-color');
const LOW_LIGHT = !given('--no-low-light');
// 0.12 is wide enough to survive 4:2:0 chroma subsampling and JPEG ringing at the boundary, and narrow enough that the middle is still most of the picture.
...
// `handleFrame` only reaches `pumpColorDecode` when a frame declares `colorBytes > 0`, so the frames decide whether the colour path
// runs at all. Both edits are needed or nothing parses - `server/capture.js` refuses a frame whose two declared lengths do not describe
// it. After the HD build and before anything emits: the build reads the source's colour block, and `encode()` reads the truncated array.
if (!COLOR) {
...
// One command per line on stdin. `low-light` is accepted and ignored because there is no device here to apply it to, and refusing it would reject a command the server legitimately sends.
let stdinPending = '';
process.stdin.on('data', (chunk) => {
  ...
  while ((nl = stdinPending.indexOf('\n')) !== -1) {
    ...
    if (line === 'hd-color on' || line === 'hd-color off') {
      // Mirrors `requestHdColor`, which never sends either command to a colour-off grabber: type 3 is the colour camera's own picture, so answering would manufacture the thing `--no-color` removes.
      if (!COLOR) {
...
// The wall clock goes in the hello and nowhere else; every frame stamp below is monotonic. `lowLight` is the conjunction and not the
// negation of its own flag - `native/grabber.cpp` reports `(wantColor && lowLight)`, so `--no-color` alone says false - and it is written
// only when something settles it, because adding a key the sample's hello does not carry would move every take's content hash.
...
// A real grabber sees EPIPE when the reader goes away first and it is not an error; unhandled it exits with a stack trace that reads as the grabber having failed.
process.stdout.on('error', (err) => { if (err.code === 'EPIPE') process.exit(0); });
// Monotonic and strictly ascending - the index, the retime curve and `mixT` rest on it. Started from the process clock so two takes in one session do not begin at the same stamp.
...
// Appended synchronously and before the bytes go out, so a writer killed mid-take has logged everything the reader could have received
// and never less. The fourth column hashes the body a reader downstream receives - a colour message's JPEG without the stamp in front of
// it - which the payload hash cannot serve as, because the stamp moves per frame.
...
const encode = () => {
  ...
  // Only the u64 at payload offset 8 moves, so the depth and the JPEG are real bytes off a real sensor.
...
// The hello and the burst leave in one write, so they arrive in one chunk and the reader's parser hands them to the recorder inside a single turn.
